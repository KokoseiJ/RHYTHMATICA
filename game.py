...
class Game:
    ...
    def init_pygame(self):
        ...
        self.screen.set_caption(self.name)
    ...
    def set_scene(self, scene):
        if not type(scene) is Scene:
            raise TypeError(f"Scene {scene} is not callable!")
        ...
        logger.debug("Scene registered, Calling start", name)
        self.scene.start()
        logger.debug("Scene started", name)

    def add_task(self, task):
        """
        Task should be a callable with one argument.
        """
        self.tasks.put(task)
